...
var reverseInParallel = (trace => {
  ...
  const logErrorOnce = once(errorLog);
  return index$1(lines).map(line => {
    return index$1(push => {
      const reverse = child_process.exec(`node dist/reverser.js`, (err, x) => {
        if (err) {
  ...
  }).parallel(lines.length).map(x => x.toString('utf8')).collect().invoke('join', ['']);
});
...
var index = (() => {
  server = http.createServer((request, response) => {
    const through = index$1.pipeline(index$1.map(x => x.toString('utf-8')), index$1.collect(), index$1.map(x => x.join('')), index$1.flatMap(x => {
      ...
      errorLog.error(xs, 'after flatmap');
